"""
傾斜区分分析ツール (北茨城市 オフライン版)

起動: python terrain_web.py
ブラウザが自動的に http://127.0.0.1:8767/ を開きます。
...
def _fill_nan(arr):
    ...
    filled[mask] = 0.0 if math.isnan(mean_val) else mean_val
    return filled
...
def pixel_sizes_at_lat(lat, zoom=15):
    # Web メルカトルは正角投影なので、ある緯度での地上分解能(m/px)は
    # 東西・南北とも同じ値（cos(lat) 補正込み）になる。
    # 以前は py に cos(lat) 補正が無く、南北方向が 1/cos(lat) ≒ 1.25 倍に
    # なって面積が約25%過大になっていた。
    earth_circ = 2 * math.pi * 6378137
    res = earth_circ * math.cos(math.radians(lat)) / (256 * 2**zoom)
    return res, res
...
def analyze_polygon(polygon_coords):
    ...
    px, py = pixel_sizes_at_lat(mid_lat, zoom=15)
...
<header>
  ...
  <div>
    <h1>傾斜区分分析ツール</h1>
    <small>北茨城市専用 オフライン版 ／ 国土地理院 DEM5A z=15（約4m解像度）</small>
  </div>
</header>
...
// ===== 地図初期化 =====
var map = L.map('map').setView([36.85, 140.67], 11);
